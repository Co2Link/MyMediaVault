...
export const { handlers, auth, signIn, signOut } = NextAuth(() => buildAuthConfig(getAuthEnv()));

function buildAuthConfig(env: AuthEnv) {
  return {
    ...
    events: {
      async signIn({ account, profile, user }) {
        ...
        const isAdmin = await resolveAdminStatus(env, entraOid, entraProfile?.groups ?? [], account?.access_token ?? null);
...
async function resolveAdminStatus(
  ...
  return checkAdminGroupMembership(env, accessToken);
}

async function checkAdminGroupMembership(env: AuthEnv, accessToken: string) {
