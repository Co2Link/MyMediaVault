...
- `develop`: active development branch. A push to `develop` runs CI, builds the
  backend image, applies the dev Terraform environment, deploys the frontend,
...
- Push to `develop` only when the user asks to deploy dev or when the requested
  change is explicitly meant to trigger the dev environment.
