...
- `apps/functions`: Azure Functions worker for queue-driven torrent metadata
  processing.
...
3. The web app writes user-owned video rows and enqueues torrent metadata work.
4. The worker resolves the torrent, stores the raw payload in blob storage, and
   writes parsed metadata back to Cosmos DB.
