...
Playwright-driven integration coverage. Torrent metadata retrieval is isolated
...
**Testing**: pytest for backend unit/integration tests; ty for backend type checking; ruff for backend linting/formatting; Vitest + Testing Library for frontend tests; Playwright CLI for end-to-end tests  
...
**Project Type**: Monorepo with `apps/backend`, `apps/frontend`, `infra/terraform`, shared docs, GitHub Actions CI/CD, and cross-app end-to-end tests  
...
**Constraints**: Use `uv add` instead of editing backend `pyproject.toml` manually; keep first delivery minimal; bypass Entra ID in automated tests; prefer established libraries over custom implementations; use Azure always-free or consumption-tier services where possible  
...
- **Testing**: Required layers are backend unit tests, backend integration tests
  with test database/storage doubles, contract checks against the OpenAPI
  contract, frontend component tests, Playwright end-to-end flows, and regression
  tests for duplicate info hash handling and user isolation.
...
risks; data-model.md defines constraints and state transitions; contracts define
API behavior; quickstart.md documents commands; no unresolved clarification
markers remain.
...
separately, while `apps/e2e` owns cross-application Playwright scenarios.
